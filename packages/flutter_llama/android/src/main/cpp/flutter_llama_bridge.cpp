...
Java_net_nativemind_flutter_1llama_FlutterLlamaPlugin_nativeGenerate(
    ...
    if (grammar != nullptr) {
        ...
        if (grammar_str != nullptr && strlen(grammar_str) > 0) {
            auto* gsampler = llama_sampler_init_grammar(g_vocab, grammar_str, "root");
            if (gsampler != nullptr) {
                llama_sampler_chain_add(g_sampler, gsampler);
            }
        }
...
Java_net_nativemind_flutter_1llama_FlutterLlamaPlugin_nativeGenerateStreamInit(
    ...
    LOGI("Stream init done, ready to generate tokens one by one");
}

// Generate and return next token one at a time
...
Java_net_nativemind_flutter_1llama_FlutterLlamaPlugin_nativeGenerateStreamNext(
    ...
    if (!g_model || !g_context || !g_vocab || !g_sampler) {
        return nullptr;
    }
    ...
    int generated = g_n_pos - g_n_prompt;
    if (generated >= g_max_tokens) {
        return nullptr;
    }
    ...
    g_n_pos++;
    if (llama_decode(g_context, batch) != 0) {
        LOGE("Failed to decode token during streaming");
        return nullptr;
    }
